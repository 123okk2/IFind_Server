...
class CACD(data.Dataset):
    def __init__(self,split="train",transforms=None, label_transforms=None):
        ...
        data_root = r"D:\Datasets\CACD2000"
        ...
        for i in range(len(group_lists)):
            with open(group_lists[i], 'r', encoding='UTF8') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            group_images = []
            for l in lines:
        ...
        if self.split is "train":
            ...
            with open(os.path.join(list_root, 'train.txt'), 'r', encoding='UTF8') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            shuffle(lines)
            for l in lines:
            ...
            self.source_images = []  # which use to aging transfer
            with open(os.path.join(list_root, 'test.txt'), 'r', encoding='UTF8') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            shuffle(lines)
            for l in lines:
