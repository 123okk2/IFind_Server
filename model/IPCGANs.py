...
class IPCGANs:
    def __init__(self,lr=0.01,generator_path=None, discriminator_path=None, age_classifier_path=None,
                 gan_loss_weight=75,feature_loss_weight=0.5e-4,age_loss_weight=30):

        self.d_lr=lr
        self.g_lr=lr
        if generator_path is not None:
            self.generator=Generator(pretrainded=True,modelpath=generator_path).cuda()
        else:
            self.generator = Generator(pretrainded=False).cuda()

        if discriminator_path is not None:
            self.discriminator=PatchDiscriminator(pretrainded=True,modelpath=discriminator_path).cuda()
        else:
            self.discriminator = PatchDiscriminator(pretrainded=False).cuda()
