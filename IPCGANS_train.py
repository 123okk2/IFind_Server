...
def main(args, logger, writer):
    ...
    model=IPCGANs(lr=args.learning_rate,age_classifier_path=args.age_classifier_path,gan_loss_weight=args.gan_loss_weight,
                  feature_loss_weight=args.feature_loss_weight,age_loss_weight=args.age_loss_weight
                  , generator_path=args.g_checkpoint, discriminator_path=args.d_checkpoint)
                  #,feature_extractor_path=args.feature_extractor_path)
    d_optim=model.d_optim
    g_optim=model.g_optim

    for epoch in range(args.max_epoches):
        avr_d_loss = 0;avr_g_loss = 0; count = 0
        ...
        content = "[202.*.*.150.]  [INFO] Epoch End : " + str(epoch) + ", d_loss : " + str(avr_d_loss) + ", g_loss : " + str(avr_g_loss)
        payload = {"text": content}
        requests.post(
            webhook_url, data=json.dumps(payload),
            headers={'Content-Type': 'application/json'}
...
def mysetting(max_epochs, gan_loss_weight=75, learning_rate=1e-4, batch_size=32, dcheck_point=None, gcheck_point=None, checkpoint=None, feature_loss_weight=0.5e-4, age_loss_weight = 30):
    ...
    # Optimizer
    parser.add_argument('--learning_rate', '--lr', type=float, help='learning rate', default=learning_rate)
    parser.add_argument('--batch_size', '--bs', type=int, help='batch size', default=batch_size)
    parser.add_argument('--max_epoches', type=int, help='Number of epoches to run', default=max_epochs)
    ...
    # Model
    parser.add_argument('--gan_loss_weight', type=float, help='gan_loss_weight', default=gan_loss_weight)
    parser.add_argument('--feature_loss_weight', type=float, help='fea_loss_weight', default=feature_loss_weight)
    parser.add_argument('--age_loss_weight', type=float, help='age_loss_weight', default=age_loss_weight)
    parser.add_argument('--age_groups', type=int, help='the number of different age groups', default=5)
    parser.add_argument('--age_classifier_path', type=str, help='directory of age classification model',
                        default='./checkpoint/pretrain_alexnet/saved_parameters/age_cls_epoch_47_iter_0.pth')
    ...
    # Data and IO
    parser.add_argument('--d_checkpoint', type=str, help='pretrained checkpoints directory',
                        default=dcheck_point)
    parser.add_argument('--g_checkpoint', type=str, help='pretrained checkpoints directory',
                        default=gcheck_point)
    parser.add_argument('--checkpoint', type=str, help='logs and checkpoints directory',
                        default=checkpoint)
...
if __name__ == '__main__':
    check_dir_1 = r'D:\Checkpoint'

    for i in range(1):
        args=None; writer=None; logger=None;
        print(" [INFO] Train ", i)
        if i is 2:
            args, writer, logger = mysetting(1, checkpoint= check_dir_1)
        elif i is 0:
            args, writer, logger = mysetting(10, gan_loss_weight=80, batch_size=64, checkpoint = check_dir_1
                                             # , gcheck_point=r'D:\Checkpoint\2019-11-14_19-09-33\saved_parameters\gepoch_0_iter_9000.pth'
                                             # , dcheck_point=r'D:\Checkpoint\2019-11-14_19-09-33\saved_parameters\depoch_0_iter_9000.pth'
                                             )
        print("Setting Completed ", i)
        try:
            main(args, logger, writer)
        except Exception as ex:
            requests.post(
                webhook_url, data=json.dumps({"text": "[202.*.*.150.] [ERROR] " + str(ex)}),
                headers={'Content-Type': 'application/json'}
            )
            print("[ERROR] ", ex)

        requests.post(
            webhook_url, data=json.dumps({"text": "[202.*.*.150.] [INFO] Training Finish! "}),
            headers={'Content-Type': 'application/json'}
        )
